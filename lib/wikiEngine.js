...
// Check if the returned title is actually relevant to what we searched
function isRelevantResult(searchTerm, returnedTitle, snippet) {
  const st = searchTerm.toLowerCase();
  const rt = returnedTitle.toLowerCase();
  
  // Direct: search term appears in title or title appears in search
  if (rt.includes(st) || st.includes(rt)) return true;
  
  // Word overlap: any search word appears in title (substring)
  const searchWords = st.split(/\s+/).filter(w => w.length > 2);
  const titleWords = rt.split(/\s+/).filter(w => w.length > 2);
  ...
  // Acronym check: search term (e.g. "MBSR") appears in the snippet text
  if (snippet && snippet.toLowerCase().includes(st)) return true;
...
async function fetchFromWiki(apiBase, searchTerm) {
  ...
  // Reject results that aren't relevant to what we searched
  if (!isRelevantResult(searchTerm, title, snippet)) return null;
...
export async function fetchWiki(query) {
  ...
  // ── TRY SIMPLE ENGLISH WIKIPEDIA FIRST — with original query ──
  let result = await fetchFromWiki(SIMPLE_API, cleanQuery);
  let usedSimple = true;

  // ── FALL BACK TO REGULAR ENGLISH — with original query ──
  if (!result) {
    result = await fetchFromWiki(FULL_API, cleanQuery);
    usedSimple = false;
  }

  // ── LAST RESORT: try stripped keywords on regular English ──
  if (!result) {
    const keywords = stripStop(tokenize(cleanQuery));
    if (keywords.length > 0 && keywords.join(" ") !== cleanQuery.toLowerCase()) {
      result = await fetchFromWiki(FULL_API, keywords.join(" "));
      usedSimple = false;
    }
  }
