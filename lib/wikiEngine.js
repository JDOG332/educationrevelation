...
function isRelevantResult(searchTerm, returnedTitle) {
  const searchWords = searchTerm.toLowerCase().split(/\s+/).filter(w => w.length > 2);
  const titleWords = returnedTitle.toLowerCase().split(/\s+/).filter(w => w.length > 2);
  
  // Check if ANY search word appears in the title (or vice versa)
  for (const sw of searchWords) {
    for (const tw of titleWords) {
      // Substring match — "dissolve" matches "dissolution", "ego" matches "ego"
...
async function fetchFromWiki(apiBase, searchTerm) {
  ...
  if (!isRelevantResult(searchTerm, title)) return null;
