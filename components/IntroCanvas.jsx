 ...
 *
 * TIMELINE (10 seconds) — ALL PHASE BOUNDARIES ARE PHI-EXACT:
 *  0.000–0.090  GENESIS      (PHIi5 = 9.0%)  — gold point breathes open
 *  0.090–0.236  EXPLOSION    (PHIi4 = 14.6%) — bodies burst from center
 *  0.236–0.618  ORGANIZATION (PHIi2 = 38.2%) — Ψ orbits stabilize, geometry materializes
 *  0.618–0.854  REVELATION   (PHIi3 = 23.6%) — peak: all systems alive, aurora, Sephirot, FOL, spiral
 *  0.854–1.000  CONVERGENCE  (PHIi4 = 14.6%) — everything collapses inward, gold pulse, black
 *  NOTE: EXPLOSION and CONVERGENCE are perfect mirror twins (both PHIi4 = 14.6%)
 */

import { useEffect, useRef } from "react";
import { PHI, PHI_INV, DOORS } from '@/lib/data.js';
import {
  computeR12,
  computeG,
...
// ── VITRUVIAN TRANSITION — the man becomes the universe ─────────
// Draws the spinning man from LandingPage continuing seamlessly,
// his Vitruvian circle expanding to fill all of space, man dissolving.
// seed = { rx, ry, rz } from last LandingPage frame
// progress = 0 (man unchanged) → 1 (man gone, circle fills screen)
function drawVitruvianTransition(ctx, W, H, seed, progress) {
  const cx = W/2, cy = H/2;
  const ease = smootherstep(progress);
  const TAU = Math.PI * 2;

  // Rotation continues at exactly LandingPage speed (0.0035 * ~60fps = 0.21 rad/s)
  const elapsedSec = progress * 0.0901699437 * TOTAL_MS / 1000; // P1 * TOTAL_MS / 1000
  const RX_SPEED = 0.21; // rad/s
  const rx = (seed.rx || 0) + elapsedSec * RX_SPEED;
  const ry = (seed.ry || 0) + elapsedSec * RX_SPEED * PHI;
  const rz = (seed.rz || 0) + elapsedSec * RX_SPEED * PHI * PHI;

  // R: circle expands from landing scale to well beyond screen edges
  const R_start = Math.min(W, H) * 0.38;
  const R_end   = Math.min(W, H) * 1.62; // PHI × screen — circle envelops you
  const R = R_start + (R_end - R_start) * ease;

  // Figure fades in first 61.8% (PHIi of progress)
  const figAlpha = Math.max(0, 1 - smootherstep(Math.min(1, progress / 0.618)));

  // ── 3D helpers — exact match to LandingPage ──
  function rotXv([x,y,z],a){return[x,y*Math.cos(a)-z*Math.sin(a),y*Math.sin(a)+z*Math.cos(a)];}
  function rotYv([x,y,z],a){return[x*Math.cos(a)+z*Math.sin(a),y,-x*Math.sin(a)+z*Math.cos(a)];}
  function rotZv([x,y,z],a){return[x*Math.cos(a)-y*Math.sin(a),x*Math.sin(a)+y*Math.cos(a),z];}
  function xfv(p){return rotZv(rotYv(rotXv(p,rx),ry),rz);}
  function projv([x,y,z]){const s=2.4/(2.4+z*0.4);return[cx+x*R*s, cy-y*R*s, z];}
  function dAv(z){return 0.15+0.75*((z+1)/2);}
  const WH=(a)=>`rgba(220,215,200,${Math.max(0,Math.min(1,a)).toFixed(3)})`;
  const GD=(a)=>`rgba(201,168,76,${Math.max(0,Math.min(1,a)).toFixed(3)})`;

  // Dark background — slightly thicker trail than landing for drama
  ctx.fillStyle=`rgba(3,3,10,0.14)`; ctx.fillRect(0,0,W,H);

  // ── Figure (fades with PHIi cutoff) ──
  if (figAlpha > 0.005) {
    const armA=Math.PI/2.6, legA=Math.PI/5.2;
    const ca=Math.cos(armA),sa=Math.sin(armA),cl=Math.cos(legA),sl=Math.sin(legA);
    const limbs=[
      [[0,.18,0],[0,0,0],[0,-.28,0]],
      [[-0.18,.10,0],[-0.18-0.22*ca,.10-0.22*sa,0],[-0.18-0.42*ca,.10-0.42*sa,0]],
      [[0.18,.10,0],[0.18+0.22*ca,.10-0.22*sa,0],[0.18+0.42*ca,.10-0.42*sa,0]],
      [[-0.10,-.28,0],[-0.10-0.15*sl,-.28-.28*cl,0],[-0.10-0.20*sl,-.28-.56*cl,0]],
      [[0.10,-.28,0],[0.10+0.15*sl,-.28-.28*cl,0],[0.10+0.20*sl,-.28-.56*cl,0]],
    ];
    limbs.forEach(pts=>{
      const m=pts.map(p=>projv(xfv(p)));
      const z=m.reduce((s,p)=>s+p[2],0)/m.length;
      ctx.beginPath(); ctx.moveTo(m[0][0],m[0][1]);
      m.slice(1).forEach(p=>ctx.lineTo(p[0],p[1]));
      ctx.strokeStyle=WH(dAv(z)*figAlpha);
      ctx.lineWidth=2.4; ctx.lineCap='round'; ctx.lineJoin='round'; ctx.stroke();
    });
    // Head
    const hc=projv(xfv([0,0.33,0])),he=projv(xfv([0.12,0.33,0]));
    const hr=Math.max(Math.abs(he[0]-hc[0]),5);
    ctx.beginPath(); ctx.arc(hc[0],hc[1],hr,0,TAU);
    ctx.strokeStyle=WH(dAv(hc[2])*figAlpha*0.84);
    ctx.lineWidth=2.2; ctx.stroke();
    // Square
    const sq=[[-0.52,.70,0],[.52,.70,0],[.52,-.70,0],[-.52,-.70,0],[-.52,.70,0]].map(p=>projv(xfv(p)));
    const sqZ=sq.reduce((s,p)=>s+p[2],0)/sq.length;
    ctx.beginPath(); ctx.moveTo(sq[0][0],sq[0][1]);
    sq.slice(1).forEach(p=>ctx.lineTo(p[0],p[1]));
    ctx.strokeStyle=GD(dAv(sqZ)*0.28*figAlpha);
    ctx.lineWidth=0.9; ctx.stroke();
  }

  // ── Circle — expands and becomes the universe ──
  const circPts=Array.from({length:65},(_,i)=>{
    const a=(i/64)*TAU;
    return projv(xfv([Math.cos(a)*0.70,Math.sin(a)*0.70,0]));
  });
  const circZ=circPts.reduce((s,p)=>s+p[2],0)/circPts.length;
  ctx.beginPath(); ctx.moveTo(circPts[0][0],circPts[0][1]);
  circPts.forEach(p=>ctx.lineTo(p[0],p[1]));
  ctx.strokeStyle=GD(dAv(circZ)*(0.42+ease*0.45));
  ctx.lineWidth=1.2+(1-figAlpha)*3.2; ctx.stroke();

  // ── Navel → Genesis point brightens as man dissolves ──
  const dotR = 2.2 + ease * 9;
  const dotA = 0.35 + ease * 0.55;
  // Pulse corona when man is mostly gone
  if (progress > 0.62) {
    const pt=smootherstep((progress-0.62)/0.38);
    const pr=pt*Math.min(W,H)*0.42;
    const grd=ctx.createRadialGradient(cx,cy,0,cx,cy,Math.max(1,pr));
    grd.addColorStop(0,`rgba(201,168,76,${(1-pt)*0.75})`);
    grd.addColorStop(0.35,`rgba(201,168,76,${(1-pt)*0.25})`);
    grd.addColorStop(1,'rgba(201,168,76,0)');
    ctx.beginPath(); ctx.arc(cx,cy,Math.max(1,pr),0,TAU);
    ctx.fillStyle=grd; ctx.fill();
  }
  const dg=ctx.createRadialGradient(cx,cy,0,cx,cy,dotR*5);
  dg.addColorStop(0,`rgba(201,168,76,${dotA})`);
  dg.addColorStop(1,'rgba(201,168,76,0)');
  ctx.beginPath(); ctx.arc(cx,cy,dotR*5,0,TAU); ctx.fillStyle=dg; ctx.fill();
  ctx.beginPath(); ctx.arc(cx,cy,dotR,0,TAU);
  ctx.fillStyle=`rgba(201,168,76,${dotA})`; ctx.fill();
}

// ── DRAW ────────────────────────────────────────────────────
function draw(ctx, state, t, elapsed, W, H, seed) {
  const cx=W/2, cy=H/2;
  const fov = 500 + Math.sin(elapsed*0.00025)*55;

  // PHI-exact phase boundaries
  const P1 = 0.0901699437; // PHIi5 — end of GENESIS
  const P2 = 0.2360679775; // PHIi3 — end of EXPLOSION
  const P3 = 0.6180339887; // PHIi  — end of ORGANIZATION
  const P4 = 0.8541019662; // PHIi + PHIi3 — end of REVELATION
  // CONVERGENCE: P4 → 1.0

  const isTransition   = t < P1;
  const isExplosion    = t >= P1 && t < P2;
  const isOrganization = t >= P2 && t < P3;
  const isConvergence  = t >= P4;

  const localT = isTransition  ? t/P1
               : isExplosion    ? (t-P1)/(P2-P1)
               : isOrganization ? (t-P2)/(P3-P2)
               : isConvergence  ? (t-P4)/(1.0-P4)
               :                  (t-P3)/(P4-P3);
  ...
  // ── TRANSITION — the Vitruvian Man becomes the universe ──────
  if (isTransition) {
    drawVitruvianTransition(ctx, W, H, seed || { rx:0, ry:0, rz:0 }, localT);
    return;
  }
...
// ── Main component ───────────────────────────────────────────
export default function IntroCanvas({ onComplete, seedState }) {
  ...
  useEffect(() => {
    ...
    function loop(now) {
      ...
      draw(ctx, stateRef.current, t, elapsed, canvas.width, canvas.height, seedState);
